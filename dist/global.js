...
;// CONCATENATED MODULE: ./src/components/Buttons/ButtonsLoadingState.js
// Button Selectors
var $footerSubmit = $('#newsletter-subscribe'); // footer opt-in

var $footerOptInForm = $('#newsletter-optin-form'); // footer opt-in form
...
var ButtonsLoadingState = function ButtonsLoadingState() {
  ...
  $footerOptInForm.submit(function () {
    ...
    $.ajax({
      ...
      success: function success(data) {
        // console.log('success');
        $footerSubmit.toggleClass(btnLoading);
        $footerSubmit.toggleClass(btnSuccess); // $this[0].reset();
      ...
      error: function error(data) {
        console.log('error');
        $footerSubmit.toggleClass(btnLoading); // temporary to be able to see how the success state looks in either case, remove
  ...
  $jobApplicationSubmit.replaceWith(jobApplicationSubmitReplacement); // prettier-ignore
  ...
  $jobApplicationForm.submit(function () {
    ...
    $.ajax({
      ...
      success: function success(data) {
        // console.log('success');
        $jobApplicationSubmit.toggleClass(btnLoading);
        $jobApplicationSubmit.toggleClass(btnSuccess); // $this[0].reset();
      ...
      error: function error(data) {
        console.log('error');
        $jobApplicationSubmit.toggleClass(btnLoading); // temporary to be able to see how the success state looks in either case, remove
...
;// CONCATENATED MODULE: ./src/components/Modals/CookiesConsent/CookiesConsent.js
...
var $acceptCookiesBtn = $('#accept-cookies');
var $cookiesConsentModal = $('.modal--cookies-consent'); // Cookie Names

var cookiesConsentCookie = 'acceptCookies'; // Functions

var cookiesConsentPrompt = function cookiesConsentPrompt() {
  ...
  gsapWithCSS.to($cookiesConsentModal, {
    y: 0,
    ease: 'power1.out',
    display: 'flex',
    scrollTrigger: {
  ...
  $acceptCookiesBtn.on('click', function () {
    ...
    js_cookie_default().set(cookiesConsentCookie, true, {
      expires: 30
...
var CookiesConsent = function CookiesConsent() {
  if (js_cookie_default().get(cookiesConsentCookie)) {
    ...
    if ($ageGate.hasClass(activeAgeGate) || window.location.href.indexOf('legal') > -1) {
...
/* harmony default export */ const CookiesConsent_CookiesConsent = (CookiesConsent);
;// CONCATENATED MODULE: ./src/components/Modals/AgeGate/AgeGate.js

 // Selectors

var $body = $('body');
var $html = $('html');
var $ageGate = $('.modal--age-gate'); // containing element

var $ageGateForm = $('.modal__form--age-gate');
var $verifyAge = $('#verify-age');
var $rememberMe = $('#remember-me');
var $onScreenCheckbox = $('.form__checkbox');
var $invalidPrompt = $('.form__caption--invalid'); // Classes

var activeAgeGate = 'modal--active';
var disableScroll = 'active-modal';
var isInvalid = 'is--invalid'; // Cookie Names

var defaultCookie = 'default';
var rememberMeCookie = 'rememberMe'; // Functions

var cookiesOrLegal = function cookiesOrLegal() {
  if (js_cookie_default().get(rememberMeCookie) || js_cookie_default().get(defaultCookie) || window.location.href.indexOf('legal') > -1) {
...
var validateAge = function validateAge() {
  if (!$verifyAge.prop('checked')) {
    ...
    js_cookie_default().set(defaultCookie, true, {
      expires: 1
    });
    ...
    js_cookie_default().set(rememberMeCookie, true, {
      expires: 30
    });
    $ageGate.removeClass(activeAgeGate);
    $html.removeClass(disableScroll);
...
var AgeGate = function AgeGate() {
  $verifyAge.attr('aria-required', 'true'); // Checks for the existence of cookies and whether or not the user is on the Legal page

  cookiesOrLegal(); // Deals with the age verification and cookie creation logic

  $ageGateForm.submit(function (e) {
    e.preventDefault();
    validateAge();
  });
};

/* harmony default export */ const AgeGate_AgeGate = (AgeGate);
;// CONCATENATED MODULE: ./src/components/Modals/ShopSegmentation/ShopSegmentation.js
// prettier-ignore
...
var ShopSegmentation_$html = $('html');
var ShopSegmentation_$body = $('body'); // Modal

var $segmentationModal = $('.modal--shop'); // containing element

var $modalContainer = $('.modal__container');
var $segmentationForm = $('.modal__form--shop');
var $storeSelect = $('#shop-segmentation-select');
var $selectPlaceholder = $('#shop-segmentation-select > option:first-child');
var $modalHeader = $('#shop-segmentation-label');
var $modalBody = $('#choose-store-label');
var $modalDesc = $('#shop-segmentation-description');
var defaultModalHeader = $modalHeader.html();
var defaultModalBody = $modalBody.html();
var defaultModalDesc = $modalDesc.html(); // Buttons

var $navShop = $('.is--shop-trigger'); // nav 'shop' link

var $heroShop = $('#shop-now-btn'); // hero 'shop now'

var $dailySpecialsShop = $('#daily-specials-btn'); // daily specials/staff picks

var $shopFlower = $('#shop-flower-btn'); // shop flower

var $shopEdibles = $('#shop-edibles-btn'); // shop edibles

var $shopVaporizers = $('#shop-vaporizers-btn'); // shop vaporizers

var $escModal = $('#modal-esc'); // explicit modal esc
// Classes

var activeSegmentation = 'modal--active';
var ShopSegmentation_disableScroll = 'active-modal'; // URL Parts
// const dutchieTopLevel = 'https://dutchie.com/embedded-menu'; // top-level

var dutchieTopLevel = '/cannabis-store-menu/'; // top-level revised
// Stores
...
var danforth = 'danforth-dispensary?dtche%5B'; // danforth main revised

var liberty = 'liberty-village-dispensary?dtche%5B'; // liberty main revised

var mountDennis = 'mount-dennis-dispensary?dtche%5B'; // mount dennis main revised
// Destinations
...
var specials = 'path%5D=specials'; // specials category revised

var flower = 'category%5D=flower'; // flower category revised

var edibles = 'category%5D=edibles'; // edibles category revised

var vaporizers = 'category%5D=vaporizers'; // vaporizers category revised
// URL Variables

var main = false;
var store;
var products = false;
var destination; // Functions

var pathConstructor = function pathConstructor(main, store, products, destination) {
  if (main) {
    // convert to switch-case?
    // return `${dutchieTopLevel}/${store}`;
    return "".concat(dutchieTopLevel).concat(store); // revised
  } else if (!products) {
    // return `${dutchieTopLevel}/${store}/${destination}`;
    return "".concat(dutchieTopLevel).concat(store).concat(destination); // revised
  } else if (products) {
...
var escClose = function escClose() {
  $document.keyup(function (e) {
    ...
    if ($segmentationModal.hasClass(activeSegmentation) && e.keyCode === escKey) {
...
var clickAwayClose = function clickAwayClose() {
  $document.on('click', function (e) {
    var $target = $(e.target);

    if ( // clicking away from the modal when modal is active
    !$target.closest($modalContainer).length && !$target.closest($navShop).length && !$target.closest($heroShop).length && !$target.closest($dailySpecialsShop).length && !$target.closest($shopFlower).length && !$target.closest($shopEdibles).length && !$target.closest($shopVaporizers).length && !$target.closest($escModal).length && // this may be buggy
    $segmentationModal.hasClass(activeSegmentation)) {
      ...
      $modalBtn.html(defaultModalBtn);
      $modalDesc.html(defaultModalDesc);
    } else if ( // clicking the modal when its active
    $target.closest($modalContainer).length && $segmentationModal.hasClass(activeSegmentation)) {
      return;
...
var explicitEsc = function explicitEsc() {
...
var ShopSegmentation = function ShopSegmentation() {
  ...
  $heroShop.on('click', function () {
    ...
    ShopSegmentation_$html.addClass(ShopSegmentation_disableScroll);
    ShopSegmentation_$body.addClass(ShopSegmentation_disableScroll); // capture value of data attr – try the alternative to 'this' from the docs

    var dataVal = $(this).data('menu'); // construct part of the url – turn this into it's own function and switch-case?
  ...
  $navShop.on('click', function () {
    ...
    $segmentationModal.addClass(activeSegmentation);
    ShopSegmentation_$html.addClass(ShopSegmentation_disableScroll);
    ShopSegmentation_$body.addClass(ShopSegmentation_disableScroll); // check if currently on a menu page
    ...
    if (window.location.href.indexOf('cannabis-store-menu') > -1) {
      ...
      $modalHeader.html('Please select another cannabis store location below'); // change the body text
  ...
  $dailySpecialsShop.on('click', function () {
    ...
    var currentDay = currentDate.getDay(); // launch the modal – convert into it's own function

    $segmentationModal.addClass(activeSegmentation);
    ShopSegmentation_$html.addClass(ShopSegmentation_disableScroll);
    ShopSegmentation_$body.addClass(ShopSegmentation_disableScroll); // change the body text
  ...
  $shopFlower.on('click', function () {
    ...
    var dataVal = $(this).data('menu'); // construct part of the url
  ...
  $shopVaporizers.on('click', function () {
    ...
    $segmentationModal.addClass(activeSegmentation);
    ShopSegmentation_$html.addClass(ShopSegmentation_disableScroll);
    ShopSegmentation_$body.addClass(ShopSegmentation_disableScroll); // capture value of data attr

    var dataVal = $(this).data('menu'); // change the body text
  ...
  $segmentationForm.submit(function (e) {
    ...
    var $selectedStore = $storeSelect.val(); // move this into it's own function?

    if ($selectedStore === 'danforth') {
      // put these store strings in constants?
      store = danforth;
      var fullPath = pathConstructor(main, store, products, destination); // window.open(fullPath); // new window
      ...
      var _fullPath = pathConstructor(main, store, products, destination); // window.open(fullPath); // new window


      window.location.href = _fullPath; // same window
      // reset URL variables
      ...
      var _fullPath2 = pathConstructor(main, store, products, destination); // window.open(fullPath); // new window


      window.location.href = _fullPath2; // same window
      // reset URL variables
...
var $window = $(window);
var Nav_$document = $(document);
var Nav_$body = $('body');
var Nav_$html = $('html'); // Nav

var $mainNav = $('nav.main-nav'); // Nav element
// Hamburger Elements

var $hamburger = $('.main-nav__hamburger'); // Containing element

var $hamburgerTop = $('div.hamburger__top'); // Top bun

var $hamburgerBottom = $('div.hamburger__bottom'); // Bottom bun
// Links

var $linksList = $('.main-nav__links-list'); // Containing element

var $dropdownParent = $('.main-nav__links-item--dropdown'); // Links with dropdown navigation modifier

var $dropdownChildren = $('.links-item__container'); // Link and chevron container

var $dropdownIcon = $('.main-nav__dropdown-icon-bounding'); // Dropdown chevron icon

var $activeDropdownIcon = $('.main-nav__dropdown-icon-bounding--active'); // Active dropdown chevron icons

var $dropdownSubMenu = $('.main-nav__sub-menu'); // Containing element of sub-menu link lists
// Classes

var Nav_disableScroll = 'active-nav';
var activeMobileNav = 'main-nav__links-list--active'; // Unused

var mainNavActiveMobile = 'main-nav--active-mobile';
var navLogoActiveMobile = 'main-nav__logo--active-mobile';
var mainNavDark = 'main-nav--dark';
var hamburgerBunsActiveMobile = 'hamburger--active-mobile'; // Unused

var activeDropdownIcon = 'main-nav__dropdown-icon-bounding--active';
var activeSubMenu = 'main-nav__sub-menu--active'; // Other

var tabletBreakpoint = 991; // If the mobile menu is left open, growing the window beyond this width will trigger a menu close
// Match media variables

var mediaQuery = '(max-width: 991px)';
var mediaQueryList = window.matchMedia(mediaQuery); // Functions
// Deals with showing and hiding the menu on smaller viewports

var mobileNavHandler = function mobileNavHandler() {
  Nav_$html.toggleClass(Nav_disableScroll);
  Nav_$body.toggleClass(Nav_disableScroll); // $linksList.toggleClass(activeMobileNav);

  if (!$linksList.hasClass(activeMobileNav)) {
    $linksList.addClass(activeMobileNav);

    if ($mainNav.hasClass('main-nav--dark-ui')) {
      gsapWithCSS.to($mainNav, {
        duration: 0.25,
    ...
    gsapWithCSS.to($hamburgerTop, {
      duration: 0.25,
      y: 7,
      rotationZ: 40
    });
    gsapWithCSS.to($hamburgerBottom, {
      duration: 0.25,
      y: -7,
      rotationZ: -40
    });
...
var subMenuHandler = function subMenuHandler() {
  $dropdownChildren.on('click', function () {
    var $this = $(this); // Checking for any open sub-menus
    // prettier-ignore

    var $openSubMenus = $this.parent().siblings($dropdownParent); // Siblings that also have sub-menu functionality

    if ($openSubMenus.find($dropdownSubMenu).hasClass(activeSubMenu)) {
      // If there are other open sub-menus, collapse them
      $openSubMenus.find($dropdownSubMenu).removeClass(activeSubMenu); // prettier-ignore

      $openSubMenus.find($dropdownSubMenu).prev($dropdownChildren).find($dropdownIcon).removeClass(activeDropdownIcon); // prettier-ignore

      gsapWithCSS.set($openSubMenus.find($dropdownSubMenu).prev($dropdownChildren).find($dropdownIcon), {
        transformOrigin: 'center'
      }); // prettier-ignore

      gsapWithCSS.fromTo($openSubMenus.find($dropdownSubMenu).prev($dropdownChildren).find($dropdownIcon), {
        duration: 0.25,
        rotation: 180
      }, {
        duration: 0.25,
    ...
    if ($this.find($dropdownIcon).hasClass(activeDropdownIcon)) {
      ...
      gsapWithCSS.set($this.find($dropdownIcon), {
        transformOrigin: 'center'
      });
      gsapWithCSS.fromTo($this.find($dropdownIcon), {
        duration: 0.25,
        rotation: 180
      }, {
        duration: 0.25,
      ...
      if ($window.width() > tabletBreakpoint && $mainNav.hasClass('main-nav--dark-ui')) {
        ...
        gsapWithCSS.to('.main-nav__logo-bounding', {
          duration: 0.25,
          color: '#000000'
        });
        gsapWithCSS.to('.main-nav__link', {
          duration: 0.25,
          color: '#000000'
        });
        gsapWithCSS.to($mainNav, {
...
var closeMenuHandler = function closeMenuHandler() {
  ...
  var escClose = function escClose() {
    Nav_$document.keyup(function (e) {
      var escKey = 27;

      if ($linksList.hasClass(activeMobileNav) && e.keyCode === escKey) {
        mobileNavHandler();
      } else if ($dropdownSubMenu.hasClass(activeSubMenu) && e.keyCode === escKey) {
        $dropdownSubMenu.removeClass(activeSubMenu);
  ...
  var windowResizeClose = function windowResizeClose() {
    // $window.on('resize', () => {
    //   if (
  ...
  var clickAwayClose = function clickAwayClose() {
    Nav_$document.on('click', function (e) {
      var $target = $(e.target);

      if ($window.width() > tabletBreakpoint && $dropdownSubMenu.hasClass(activeSubMenu) && !$target.closest($mainNav).length) {
        $dropdownSubMenu.removeClass(activeSubMenu);
  ...
  var scrollAwayClose = function scrollAwayClose() {
    var lastScroll = 0;
    $window.scroll(function () {
      ...
      if (currentScroll > lastScroll && $dropdownSubMenu.hasClass(activeSubMenu)) {
        // Scrolling down
        //prettier-ignore
        $dropdownSubMenu.removeClass(activeSubMenu);
        ...
        return;
      } else if (currentScroll < lastScroll && $dropdownSubMenu.hasClass(activeSubMenu) && currentScroll === 0) {
        $dropdownSubMenu.removeClass(activeSubMenu);
...
var Nav = function Nav() {
  ...
  subMenuHandler(); // Menu close functionalities
...
var GlobalComponents = function GlobalComponents() {
  Buttons_ButtonsLoadingState();
  AgeGate_AgeGate();
  ShopSegmentation_ShopSegmentation();
  CookiesConsent_CookiesConsent();
  Nav_Nav();
};
